...
[TIP]
====
👉 kafka connect / connector 是基于社区DBZ，我们需要先看一下架构图。需要注意的是，kafka connect / connector 版本变化比较快，本次实验用的2020-Q2版本, 但是现在实际上已经到了2022-Q3版本了。

image::images/2022-07-29T07-07-A08-531Z.png[] 

从上面这个图，我们就可以看出来，这里面部署的 kafka connect 是左边的组件，负责从源数据库里面抽取数据，转化为 kafka 消息，然后落到 kafka 里面去。

kafka connect 启动以后，会在 user1 project 下面运行一个新的 pod 
debezium-connect-*
====
...
[TIP]
====
👉 这里就真正 kafka connector， 他会从目标数据库拿到数据增量，并且转化为 kafka 的消息发送出去。实际上， kafka connector 是 kafka connect 的一个配置。

部署完 kafka connector，我们发现并没有新的 pod 创建，而是之前的 pod debezium-connect-* 得到了新的配置，创建了 sqlserver 抓取的thread

2022-07-29 09:04:03,919 INFO [orders-connector|task-0] Creating thread debezium-sqlserverconnector-user1.earth-change-event-source-coordinator (io.debezium.util.Threads) [task-thread-orders-connector-0]

这里面就会有一个问题，如果我们这个实验，是多人的集群环境，会部署多个 kafka connect，每个学员一个，那么这些 kafka connect 之间会不会冲突，也就是数据库的增量，是被所有的 kafka connect 给抓取了吗？

这个问题，可以慢慢探索，目前作者所知道的是，这个是 kafka connect 框架提供的功能，他给 kafka connect 提供了一套 offset 维护机制，让 connector 知道自己同步到哪里了。
====
...
[TIP]
====
👉 不得不说，这个web应用挺吓人的，他是ERP领域的应用，在浏览器里面模拟了终端字符界面的操作。

这个应用会把数据写入到后台的数据库中，后面我们就会展示，kafka connector 如何把这个更新的数据，抓取和传递出来。
====
...
[TIP]
====
👉 接下来，我们要做的事情，是部署一个新的 kafka 集群 moon， 然后把数据同步到新的集群里面去。这里面模拟的就是两个不同的地域，他们之间同步kafka的数据，做到异地的灾备，或者跨异地的业务流。
====
...
[TIP]
====
👉 我们部署一个 kafka ，注意，这里面的存储是临时性的，如果重启，kafka信息都会丢失。

然后就引出了一个问题，信息进入了kafka，你就能认为信息就不会丢失，肯定能到达目的地了呢？

这个问题，每个人的看法不同。作者认为，虽然kafka能够配置信息永远不过期，但是从系统整体架构看，kafka更多的还是信息流和流式计算，并不能当作数据库一样的组件，依赖他来存储信息。一般还是应该在相关的业务逻辑中，提供原始业务数据的存储，并且要具备信息丢失，信息重复的鉴别和数据恢复，补录的能力。
====
...
[TIP]
====
👉 这里面提到的 mirror maker 和 mirror maker 2 是什么关系，有什么联系？ 官方文档在这里： https://access.redhat.com/documentation/en-us/red_hat_amq_streams/2.1/html-single/amq_streams_on_openshift_overview/index#configuration-points-topic_str

image::images/2022-07-29T15-39-A29-792Z.png[] 

简单说，mirror maker 2 是升级版本，平常用他就好了。 mirror maker 也可以用，也是支持的，我们这个教程用的是 mirror maker。
====
...
[TIP]
====
👉 按照文档的说法，mirror maker 用的同步技术比较“原生”，是用的 kafka 本身的 consumer / producer 技术。
====
...
[TIP]
====
👉 kafka bridge 相关的官方文档，这里是整体介绍： https://access.redhat.com/documentation/en-us/red_hat_amq/2021.q3/html-single/amq_streams_on_openshift_overview/index#overview-components-kafka-bridge_str

image::images/2022-07-31T14-12-3A38-377Z.png[] 

关于 kafka bridge 的官方配置文档，在这里： https://access.redhat.com/documentation/en-us/red_hat_amq/2021.q3/html-single/amq_streams_on_openshift_overview/index#configuration-points-bridge_str
====
